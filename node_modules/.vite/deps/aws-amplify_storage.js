import {
  ...
  withMemoization
} from "./chunk-NO76UA3H.js";
import {
  fromUtf8
} from "./chunk-ZPE22TGW.js";
import {
  __commonJS,
  __toESM
...
// node_modules/@aws-amplify/storage/dist/esm/providers/s3/utils/client/s3data/deleteObjects.mjs
var import_fast_xml_parser14 = __toESM(require_fast_xml_parser(), 1);
var import_buffer15 = __toESM(require_buffer(), 1);

// node_modules/@aws-amplify/storage/dist/esm/providers/s3/utils/generateDeleteObjectsXml.mjs
var escapeXml = (str) => {
  return str.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&apos;");
};
var generateDeleteObjectsXml = (objects, quiet) => {
...
// node_modules/@aws-amplify/storage/dist/esm/providers/s3/utils/client/s3data/deleteObjects.mjs
var deleteObjectsSerializer = async (input, endpoint) => {
  const url = new AmplifyUrl(endpoint.url.toString());
...
var deleteObjectsDeserializer = async (response) => {
  if (response.statusCode >= 300) {
  ...
  for (const match of errorMatches) {
    ...
    errors.push({
      Key: keyMatch == null ? void 0 : keyMatch[1],
      Code: codeMatch == null ? void 0 : codeMatch[1],
      Message: messageMatch == null ? void 0 : messageMatch[1]
    });
  }
  const result = {
    Deleted: deleted,
...
var deleteObjects = composeServiceApi(s3TransferHandler, deleteObjectsSerializer, deleteObjectsDeserializer, { ...defaultConfig, responseType: "text" });
...
var getContentType = (data, key) => {
  ...
  const ext = (_a = key.split(".").pop()) == null ? void 0 : _a.toLowerCase();
...
var putObjectJob = (uploadDataInput, abortSignal, totalLength) => async () => {
  ...
  const { contentDisposition, contentEncoding, contentType = (uploadDataOptions == null ? void 0 : uploadDataOptions.contentType) ?? getContentType(data, objectKey) ?? "application/octet-stream", preventOverwrite, metadata, checksumAlgorithm, onProgress, expectedBucketOwner, cacheControl } = uploadDataOptions ?? {};
...
var getMultipartUploadHandlers = (uploadDataInput, size) => {
  ...
  const startUpload = async () => {
    ...
    const { contentDisposition, contentEncoding, contentType = (uploadDataOptions == null ? void 0 : uploadDataOptions.contentType) ?? getContentType(data, objectKey) ?? "application/octet-stream", metadata, preventOverwrite, onProgress } = uploadDataOptions ?? {};
    finalKey = objectKey;
...
var downloadDataJob = (downloadDataInput, abortSignal) => async () => {
  ...
  const { Body: body, LastModified: lastModified, ContentLength: size, ETag: eTag, Metadata: metadata, VersionId: versionId, ContentType: contentType } = await getObject({
    ...
    ResponseCacheControl: downloadDataOptions == null ? void 0 : downloadDataOptions.cacheControl,
...
// node_modules/@aws-amplify/storage/dist/esm/providers/s3/utils/deleteFolderContents.mjs
var MAX_KEYS_PER_BATCH = 1e3;
var deleteFolderContents = async (params) => {
  var _a, _b;
  ...
  try {
    ...
    do {
      ...
      const deleted = ((_a = deleteResult.Deleted) == null ? void 0 : _a.map((obj) => ({ path: obj.Key }))) || [];
      const failed = ((_b = deleteResult.Errors) == null ? void 0 : _b.map((err) => ({
        path: err.Key,
        code: err.Code,
        message: err.Message
      }))) || [];
...
// node_modules/@aws-amplify/storage/dist/esm/providers/s3/utils/resolveFinalKey.mjs
var resolveFinalKey = (inputType, objectKey, keyPrefix) => {
  return inputType === STORAGE_INPUT_KEY ? `${keyPrefix}${objectKey}` : objectKey;
};

// node_modules/@aws-amplify/storage/dist/esm/providers/s3/utils/validateRemovePath.mjs
var validateRemovePath = (path) => {
...
// node_modules/@aws-amplify/storage/dist/esm/providers/s3/utils/isPathFolder.mjs
var isPathFolder = async (params) => {
...
// node_modules/@aws-amplify/storage/dist/esm/providers/s3/utils/createAbortableTask.mjs
...
var executeRemove = (amplify, input) => async (abortController) => {
  var _a, _b, _c, _d, _e;
  try {
    ...
    if (isFolder) {
      return deleteFolderContents({
        ...
        bucket,
        folderKey: finalKey,
        expectedBucketOwner: (_c = input.options) == null ? void 0 : _c.expectedBucketOwner,
        onProgress: (_d = input.options) == null ? void 0 : _d.onProgress,
        abortSignal: abortController.signal
      });
    } else {
      ...
      await deleteObject({
        ...
        Bucket: bucket,
        Key: finalKey,
        ExpectedBucketOwner: (_e = input.options) == null ? void 0 : _e.expectedBucketOwner
      });
      const result = inputType === STORAGE_INPUT_KEY ? { key: objectKey } : { path: objectKey };
      return result;
    }
  } catch (error) {
    if (abortController.signal.aborted) {
...
var serviceCopy = async ({ source, destination, bucket, s3Config, notModifiedSince, eTag, expectedSourceBucketOwner, expectedBucketOwner, tagConfig }) => {
  await copyObject({
    ...s3Config,
    ...
    TaggingDirective: (tagConfig == null ? void 0 : tagConfig.mode) === "remove" ? "REPLACE" : "COPY",
...
var import_fast_xml_parser23 = __toESM(require_fast_xml_parser(), 1);
...
var getUrl = async (amplify, input) => {
  ...
  return {
    url: await getPresignedGetObjectUrl({
      ...
      ResponseCacheControl: getUrlOptions == null ? void 0 : getUrlOptions.cacheControl,
...
*/
//# sourceMappingURL=aws-amplify_storage.js.map
